...
export default function Teams() {
  ...
  const filtered = TEAMS.filter((t, idx, arr) =>
    arr.findIndex(x => x.name === t.name) === idx
  ).filter(t => {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          ...
          <div className="flex gap-2 flex-wrap">
            {confederations.map(c => (
              <button key={c} onClick={() => setActiveConf(c)}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {filtered.map(team => (
            <div key={team.name} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 hover:shadow-md transition-shadow text-center cursor-pointer">
              <div className="flex justify-center mb-3">
                <TeamFlag teamName={team.name} size={40} />
              </div>
